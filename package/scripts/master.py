...
class Master(Script):
  def install(self, env):
  	...
  	if not os.path.exists(params.temp_file):
  	  ...
  	  Execute('rm redis.conf'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  ...
  	  Execute('mkdir -p /etc/redis'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('mkdir -p /var/lib/redis'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('cp redis-server /usr/local/bin'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('cp redis.conf /etc/redis'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('mv redis-server /etc/init.d'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('chmod 755 /etc/init.d/redis-server'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('chkconfig --add redis-server'+ ' -a ' + params.redis_log_file, user=params.redis_user)
  	  Execute('chkconfig --level 345 redis-server on'+ ' -a ' + params.redis_log_file, user=params.redis_user)
