...
class Master(Script):
  def install(self, env):
  	...
  	if not os.path.exists(params.temp_file):
  	  Execute('wget ' + stable_package + ' -O ' + params.temp_file + ' -a ' + params.redis_log, user=params.redis_user)
  	  Execute('tar xvzf ' + params.temp_file+' -C ' + params.redis_install_dir + ' >> ' + params.redis_log, user=params.redis_user)
  ...
  def stop(self, env):
	  import params
	  import status_params
	  Execute('service redis stop >>' + params.redis_log, user= params.redis_user)
  ...
  def start(self, env):
	  ...
	  Execute('service redis-server start >>' + params.redis_log, user=params.redis_user)
	
  def status(self, env):
	  import params
	  import status_params
	  check_process_status(status_params.redis_pid_file)
	  Execute('service redis status >>' + params.redis_log, user=params.redis_user)
